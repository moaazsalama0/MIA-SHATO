...
async def process_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Avoid logging the full base64 audio payload (it can be very large / binary-like)
        debug_response = dict(response)
        if debug_response.get("tts_audio_base64"):
            b64 = debug_response["tts_audio_base64"]
            debug_response["tts_audio_base64"] = f"<base64 {len(b64)} chars truncated>"
        print(f"[DEBUG] Final response: {debug_response}")
        return response
